...
export default function BookClient({
  ...
  useEffect(() => {
    ...
    channel.bind("booking_created", (data) => {
      ...
      if (data.location === selectedLocation) {
        ...
        showToast(
          `Slot ${data.slotid} booked at ${data.hour}:00–${data.hour + 1}:00`,
          "info"
        );
      }
    });
  ...
  const handleWalletBooking = async (slot) => {
    ...
    try {
      ...
      showToast(
        `Booked slot ${slot.slotid} at ${selectedHour}:00–${
          selectedHour + 1
        }:00. New balance: ₹${res.data.newBalance}`,
        "success"
      );
...
const handleUPIBooking = async (slot) => {
  ...
  useEffect(() => {
    if (selectedLocation !== location) {
      router.push(`/book?location=${selectedLocation}`, undefined, {
        shallow: true,
      });
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-indigo-50 to-blue-50">
      ...
      {error && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
          <div
            className="bg-red-50 border-l-4 border-red-600 rounded-lg p-4 shadow-md"
            role="alert"
            aria-live="assertive"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {loading ? (
          ...
          <div
            className="bg-yellow-50 border-l-4 border-yellow-600 rounded-lg p-5 text-center shadow-sm"
            role="alert"
          >
            <svg
              className="w-10 h-10 text-yellow-600 mx-auto mb-2"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
            ...
            <p className="text-yellow-800 font-semibold text-lg">
              No slots available in {selectedLocation}
            </p>
            <p className="text-yellow-700 mt-1">
              Try selecting another location or contact support.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {slots.map((slot) => {
              ...
              const bookedHoursToday = (
                Array.isArray(slot.bookedHours) ? slot.bookedHours : []
              )
                .filter(
                  (bh) =>
                    bh.date &&
                    (bh.date.toISOString
                      ? bh.date.toISOString().split("T")[0] === today
